...
def process_episode(
    ...
    for row_idx, row in tqdm(df.iterrows(), total=len(df), 
        ...
        frame_data = {
            ...
            "is_first": np.array([frame_index == 0], dtype=bool),
            "is_last": np.array([False], dtype=bool),       # 末帧后面再设 True
            "is_terminal": np.array([False], dtype=bool),   # 末帧后面再设 True
            "is_episode_successful": np.array([not config.unsuccessful], dtype=bool),
        }
    ...
    episode_frames[-1]["is_last"] = np.array([True],dtype=bool)
    episode_frames[-1]["is_terminal"] = np.array([True],dtype=bool)
